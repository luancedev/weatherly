...
export function BarLocation({ city, country, state }: Props) {
  ...
  useEffect(() => {
    const timeData = setInterval(() => setHora(formatTime()), 30000);
    return () => clearInterval(timeData);
  }, []);

  const cleanStateName = (state?: string) => {
    if (!state) return '';
  ...
  const stateName = cleanStateName(state);
  ...
  return (
    <div className="flex rounded-b-xl justify-between w-full px-2 sm:px-5 py-2 sm:py-3 bg-black/20 items-center flex-col sm:flex-row gap-2 sm:gap-0">
      <p className="flex w-full gap-1 sm:gap-1.5 text-white font-medium items-center text-xs sm:text-sm lg:text-2xl">
        <span>
          <svg
